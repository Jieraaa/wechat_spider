...
module.exports = {
    ...
    savePosts: function (rawList) {
        ...
        list.forEach(function (item) {
            ...
            multiList.forEach(function (item) {
                var idx = item.content_url.replace(/amp;/g, "").split('?')[1].split('&').map(function (n) {
    ...
    replaceServerResDataAsync: function (req, res, serverResData, callback) {
        ...
        else if (/mp\/profile_ext\?action=home/i.test(req.url)) {//列表页
            try {
                var reg = /var msgList = \'(.*?)\';/;
                var ret = reg.exec(serverResData.toString());
                ...
                var nextUrl = this.getNextUrl(req.url, ret);
								//
								// if (nextUrl) {
                 //    // var next = this.getNextChunk(nextUrl, 6000);
                 //    var note = that.getNotification();
                 //    serverResData = note + serverResData + next;
                 //    //callback(serverResData);
                 //    callback(serverResData);
        ...
        else if (/mp\/profile_ext\?action=getmsg/i.test(req.url)) {//列表页
            try {
                var msg=JSON.parse(serverResData.toString());
                if (!msg.general_msg_list) {
                    callback(serverResData);
                    return;
                }
                this.savePosts(msg.general_msg_list.toString())
                callback(serverResData);
            }
            catch (e) {
                console.log(e);
        ...
        else if (/mp\/getappmsgext\?__biz/i.test(req.url)) {//打开文章
            try {
